...
class Enhanced_Channel_Attention(nn.Module):
    def __init__(self, gate_channels, reduction_ratio=16):
        super(Enhanced_Channel_Attention, self).__init__()
    ...
    def forward(self, x):
        ...
        x_avg = self.avg_pool(x).view(b, -1).unsqueeze(1).unsqueeze(1)
        x_max = self.max_pool(x).view(b, -1).unsqueeze(1).unsqueeze(1)
        x_cat = torch.cat((x_avg, x_max), dim=2)
        x_cat = self.down_conv(x_cat)
...
class CLAM(nn.Module):
    def __init__(self, gate_channels, reduction_ratio=16, no_spatial=False):
        ...
        self.ChannelGate = Enhanced_Channel_Attention(gate_channels, reduction_ratio)
        self.no_spatial = no_spatial
        if not no_spatial:
            self.SpatialGate = SpatialGate()

    def forward(self, x):
        x_out1 = self.ChannelGate(x).unsqueeze(2).unsqueeze(2)
